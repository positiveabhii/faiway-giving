...
import { Search, Plus, Edit2, Trash2, Star, Image as ImageIcon } from "lucide-react";
...
export default function AdminCharitiesPage() {
  ...
  const handleAddNew = () => {
    setCurrentEdit({
      ...
      mission: "",
      image: "",
      stats: { totalRaised: "$0", upcomingEvents: 0 },
      tags: []
    });
    setIsEditing(true);
  };
      ...
      {!isEditing ? (
            ...
            {charities.map((charity, index) => (
              <div key={charity.id} className="bg-charcoal-950 border border-white/5 rounded-xl overflow-hidden flex flex-col group relative">
                ...
                <div className="h-40 relative">
                  <img src={charity.image} alt={charity.name} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto flex justify-between items-end border-t border-white/5 pt-4">
                    <div>
                      ...
                      <p className="text-emerald-400 font-bold">{charity.stats.totalRaised}</p>
                    </div>
                    <div className="flex space-x-2">
                      ...
                      <button className="w-8 h-8 rounded bg-white/5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 flex items-center justify-center transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="bg-charcoal-900 border border-white/5 rounded-xl p-6 max-w-3xl">
          <h2 className="text-xl font-bold text-white mb-6">
            {currentEdit?.id.startsWith('c') && currentEdit?.name ? "Edit Partner Profile" : "Add New Partner"}
          </h2>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" defaultValue={currentEdit?.name} className="w-full bg-charcoal-950 border border-white/10 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-gold-500/50" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Category Tags (comma separated)</label>
                  <input type="text" defaultValue={currentEdit?.tags?.join(", ")} className="w-full bg-charcoal-950 border border-white/10 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-gold-500/50" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-white/10 rounded-lg h-32 flex flex-col items-center justify-center relative hover:border-gold-500/50 transition-colors cursor-pointer group">
                  {currentEdit?.image ? (
                    <>
                      <img src={currentEdit.image} className="absolute inset-0 w-full h-full object-cover rounded-lg opacity-40 group-hover:opacity-20 transition-opacity" />
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-gray-500 text-xs">Click to upload High-Res Image</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div>
              ...
              <textarea rows={4} defaultValue={currentEdit?.mission} className="w-full bg-charcoal-950 border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-gold-500/50 resize-none"></textarea>
            </div>
            ...
            <div className="bg-charcoal-950 p-4 rounded-lg border border-white/5 flex items-center justify-between">
              ...
              <div className="w-12 h-6 bg-charcoal-800 rounded-full relative cursor-pointer border border-white/10">
                <div className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full transition-transform ${currentEdit?.id === 'c1' ? 'translate-x-6 bg-gold-400' : 'bg-gray-500'}`}></div>
              </div>
            </div>

            <div className="pt-6 border-t border-white/5 flex justify-end space-x-3">
              <button onClick={() => setIsEditing(false)} className="px-6 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-colors">
                Cancel
              </button>
              <button onClick={() => setIsEditing(false)} className="px-6 py-2 rounded-lg text-sm font-bold bg-white text-charcoal-950 hover:bg-gray-200 transition-colors">
                Save Profile
              </button>
            </div>
          </div>
