...
import { CreditCard as CardIcon } from "lucide-react"; // Use icons from lucide-react

export default function SignupPage() {
  const { session, initialized, signup, user: authUser } = useAuth();
  ...
  useEffect(() => {
    ...
    if (initialized && session && !isPaymentInProgress) {
      // If user is already active, go to dashboard
      if (authUser?.status === 'active') {
        router.replace(authUser?.role === 'admin' ? '/admin' : '/dashboard');
      }
    }
  }, [initialized, session, authUser, router, isPaymentInProgress]);
  ...
  const handleNext = async (e: React.FormEvent) => {
    ...
    if (step < 3) {
      ...
      try {
        ...
        // 1. Create account (membership status will be 'pending_payment')
        await signup(payload);
        
        // Mark payment as in progress to prevent auto-redirection by useEffect
        setIsPaymentInProgress(true);
        
        // 2. Create Razorpay Order via backend
        const { createPaymentOrder } = await import("@/lib/api/payments");
        const orderRes = await createPaymentOrder({ plan: payload.plan });
        ...
        const options = {
          ...
          name: "Fairway Giving",
          description: `${payload.plan.charAt(0).toUpperCase() + payload.plan.slice(1)} Membership`,
          ...
          modal: {
            ondismiss: () => {
              setLoadingPayment(false);
              setIsPaymentInProgress(false);
              // If payment dismissed, they are technically logged in but inactive.
              // Redirect them to profile/billing to complete payment.
  ...
  return (
    <main className="min-h-screen bg-charcoal-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl relative z-10">
        ...
        {isPaymentInProgress ? (
          <GlassCard className="p-12 text-center border-gold-500/20">
            <div className="w-20 h-20 bg-gold-500/10 rounded-full flex items-center justify-center mx-auto mb-8 animate-pulse">
              <CardIcon className="text-gold-500" size={40} />
            </div>
            <h2 className="text-3xl font-heading font-bold text-white mb-4">Complete Payment</h2>
            <p className="text-gray-400 mb-8 max-w-sm mx-auto">Please complete the payment in the secure Razorpay popup to activate your membership.</p>
            <Loader2 className="animate-spin text-gold-500 mx-auto" size={32} />
          </GlassCard>
        ) : (
          <>
              ...
              <form onSubmit={handleNext}>
                ...
                <div className="flex justify-between items-center mt-10 space-x-4">
                  ...
                  <Button
                    type="submit"
                    fullWidth={step === 1}
                    className={`flex-1 flex items-center justify-center space-x-2 py-4 ${(step === 2 && !charityId) || loadingPayment ? 'opacity-50 pointer-events-none' : ''}`}
                    disabled={loadingPayment}
                  >
                    {loadingPayment ? (
